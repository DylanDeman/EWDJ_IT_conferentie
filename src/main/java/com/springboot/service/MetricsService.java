...
public class MetricsService {
    ...
    public MetricsService(MeterRegistry registry, EventRepository eventRepository, UserRepository userRepository) {
        ...
        this.eventCreationTimer = Timer.builder("event.creation.time")
                .description("Time taken to create an event")
                .register(registry);
        this.eventUpdateTimer = Timer.builder("event.update.time")
                .description("Time taken to update an event")
                .register(registry);
        this.eventDeletionTimer = Timer.builder("event.deletion.time")
                .description("Time taken to delete an event")
                .register(registry);
        this.eventViewTimer = Timer.builder("event.view.time")
                .description("Time taken to view an event")
                .register(registry);
    }
    ...
    private void updateFavoritesTotal() {
        long totalFavorites = userRepository.findAll().stream()
                .mapToLong(user -> user.getFavoriteEvents().size())
                .sum();
